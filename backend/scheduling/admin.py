...
class TeacherAvailabilityAdmin(admin.ModelAdmin):
    list_display = (
        'teacher', 'get_weekday_display', 'start_time', 
        'end_time', 'is_active'
    )
...
class ScheduledClassAdmin(admin.ModelAdmin):
    list_display = (
        'student', 'teacher', 'date', 'start_time', 
        'end_time', 'status'
    )
    list_filter = ('status', 'date', 'teacher')
    search_fields = (
        'student__username', 'teacher__username', 
        'student__first_name', 'teacher__first_name',
        'topic'
    )
    ...
    fieldsets = (
        (None, {
            'fields': (
                'student', 'teacher', 'date', 'start_time', 
                'end_time', 'status', 'topic'
            )
        }),
        (_('Detalhes'), {
            'fields': ('notes', 'meeting_link', 'feedback')
        }),
...
class ClassNotificationAdmin(admin.ModelAdmin):
    list_display = (
        'scheduled_class', 'recipient', 'notification_type', 
        'sent_at', 'read'
    )
    list_filter = ('notification_type', 'read', 'sent_at')
    search_fields = (
        'recipient__username', 'recipient__first_name', 
