    ...
    def clean(self):
        ...
        if self.start_time and self.end_time and self.start_time >= self.end_time:
            raise ValidationError({
                'start_time': _('O horário de início deve ser anterior ao horário de término.')
            })
...
class ScheduledClass(models.Model):
    ...
    topic = models.CharField(
        _('tema'),
        max_length=200,
        help_text=_('Tema/assunto a ser trabalhado na aula')
    )
    ...
    feedback = models.TextField(
        _('feedback'),
        blank=True,
        help_text=_('Feedback do professor após a aula')
    )
